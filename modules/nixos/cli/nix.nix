...
{
  sops.secrets = {
    "nix/secret-key" = lib.optionalAttrs enableSopsNix {
    ...
    "nix/extra-options.conf" = lib.optionalAttrs enableSopsNix {
  ...
  nix = {
    settings = {
      ...
      secret-key-files = lib.optionalAttrs enableSopsNix [
        config.sops.secrets."nix/secret-key".path
      ];
    };
    # 通过 !include 包含运行时生成的配置文件(宽容模式, 如果指定的文件不存在 Nix 会忽略该指令)
    extraOptions = lib.optionalAttrs enableSopsNix "!include ${
